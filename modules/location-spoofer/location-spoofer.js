...
(function () {
  "use strict";
      ...
      var CITIES_CACHE_KEY = "SR_SPOOF_CITIES_CACHE";
  var CITIES_REMOTE_URL = "https://raw.githubusercontent.com/bkmz735/shadowrocket-modules/main/modules/location-spoofer/cities.json";
  var CITY_PRESETS = (function () {
    try {
      if (typeof require === "function") {
        try {
          var imported = require("./cities.json");
          if (imported && typeof imported === "object") return imported;
        } catch (e1) {
          try {
            var importedJs = require("./cities.js");
            if (importedJs && typeof importedJs === "object") return importedJs;
          } catch (e2) {}
        }
      }
    } catch (e) {}
    try {
      ...
      if (store) {
        var cached = store.read(CITIES_CACHE_KEY);
  ...
  function syncRemoteCitiesDatabase(debug) {
    try {
      var client = (typeof $httpClient !== "undefined") ? $httpClient : null;
      var store = (typeof $persistentStore !== "undefined") ? $persistentStore : null;
      if (!client || !store) return;
      client.get({ url: CITIES_REMOTE_URL, timeout: 5000 }, function (err, resp, body) {
        if (!err && body) {
          try {
            var parsed = JSON.parse(body);
            if (parsed && typeof parsed === "object") {
              store.write(body, CITIES_CACHE_KEY);
              CITY_PRESETS = parsed;
              if (debug) console.log("Location spoofer cities database updated: " + Object.keys(parsed).length + " presets");
            }
          } catch (eParse) {}
        }
      });
...
var DEFAULT_CONFIG = {
  ...
  function loadRuntimeConfig(callback) {
    ...
    if (Object.keys(CITY_PRESETS).length === 0) { syncRemoteCitiesDatabase(debug); }
    if (!configUrl) {
      finish();
      return;
    }
